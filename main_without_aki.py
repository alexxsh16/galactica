...
toggle_cooldowns = {}  # Dictionary to keep track of last toggle time for each user
...
async def group_join(interaction: discord.Interaction, group: str):
    ...
    group_file_path = os.path.join(groups_folder, f"{group}.json")
    user_id = str(interaction.user.id)
    
    # Check if the user is on cooldown
    if user_id in toggle_cooldowns and toggle_cooldowns[user_id] > datetime.now():
        cooldown_time = toggle_cooldowns[user_id] - datetime.now()
        minutes = cooldown_time.total_seconds() // 60
        await interaction.response.send_message("You are on cooldown. Please wait one minute before joining again.", ephemeral=True)
        return
    
    # Check if the group file exists
    if not os.path.exists(group_file_path):
        await interaction.response.send_message(f"The group '{group}' does not exist.", ephemeral=True)
        return
    
    with open(group_file_path, "r") as file:
        group_data = json.load(file)
    
    if user_id in group_data and group_data[user_id]["participating"]:
        # User has already joined
        await interaction.response.send_message(f"You are already a participant of {group}.", ephemeral=True)
    else:
        # User is joining for the first time or rejoining
        group_data[user_id] = {"participating": True}
        await interaction.response.send_message(f"You have successfully joined {group}!", ephemeral=True)
    
    # Set the toggle cooldown for the user
    toggle_cooldowns[user_id] = datetime.now() + timedelta(minutes=1)
    
    # Save the updated group data to JSON file
...
async def group_leave(interaction: discord.Interaction, group: str):
    ...
    if user_id in toggle_cooldowns and toggle_cooldowns[user_id] > datetime.now():
        cooldown_time = toggle_cooldowns[user_id] - datetime.now()
        minutes = cooldown_time.total_seconds() // 60
        await interaction.response.send_message(f"You are on cooldown. Please wait one minute before leaving again.", ephemeral=True)
        return
    ...
    if user_id in group_data and group_data[user_id]["participating"]:
        # User has already joined, mark as not participating
        group_data[user_id]["participating"] = False
        await interaction.response.send_message(f"You have successfully left {group}.", ephemeral=True)
    else:
        # User is not a participant
        await interaction.response.send_message(f"You are not a participant of {group}.", ephemeral=True)
...
async def group_ping(interaction: discord.Interaction, value: str, group: str):
    ...
    if any(role.id in allowed_roles for role in interaction.user.roles):
        ...
        # Filter the participating members
        participants = [user_id for user_id, data in group_data.items() if data["participating"]]
        
        members = [interaction.guild.get_member(int(user_id)) for user_id in participants]
        members = [member.mention for member in members if member is not None]
        ...
        if members:
            ...
            message = f"**@{group}:** {', '.join(members)}"
            await interaction.channel.send(message)
        else:
            await interaction.response.send_message(f"There are no members in {group}.", ephemeral=True)
    else:
...
async def group_check(interaction: discord.Interaction, group: str):
    ...
    if any(role.id in allowed_roles for role in interaction.user.roles):
        ...
        if members:
            message = f"Members in **@{group}:** {', '.join(members)}"
